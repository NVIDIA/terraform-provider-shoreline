...
type ElasticDataAdapter struct{}

// Ensure the adapter implements the IntegrationDataAdapter interface
var _ adapterinterface.IntegrationDataAdapter = &ElasticDataAdapter{}

func (a *ElasticDataAdapter) DataFieldNames() []string {
	return []string{"api_token", "url"}
}

func (a *ElasticDataAdapter) TFModelFieldNames() []string {
	return []string{"api_key", "api_url"}
}

func (a *ElasticDataAdapter) MapToTFModel(requestContext *common.RequestContext, options *adapterinterface.IntegrationDataAdapterOptions, integrationData map[string]interface{}, tfModel *model.IntegrationTFModel) {

	modelupdater.NewModelUpdater(options, tfModel).
		UpdateStringField("api_key", &tfModel.APIKey, utils.GetStringOrEmpty(requestContext, integrationData, "api_token")).
		UpdateStringField("api_url", &tfModel.APIUrl, utils.GetStringOrEmpty(requestContext, integrationData, "url"))
}

func (a *ElasticDataAdapter) TFModelToMap(requestContext *common.RequestContext, options *adapterinterface.IntegrationDataAdapterOptions, tfModel *model.IntegrationTFModel) map[string]interface{} {
