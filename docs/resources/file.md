...
- `checksum` (String) Cryptographic hash (e.g. md5) of a File Resource. This is used to verify the integrity of the file when it is uploaded.
- `file_data` (String) The destination URL where the file was uploaded to.
- `file_length` (Number) Length, in bytes, of a distributed File object.
